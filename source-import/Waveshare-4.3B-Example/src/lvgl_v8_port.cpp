...
static lv_disp_t *display_init(LCD *lcd)
{
    ...
    ESP_UTILS_LOGI("LVGL display init: LCD handle OK");

    static lv_disp_draw_buf_t disp_buf;
    static lv_disp_drv_t disp_drv;

    // Alloc draw buffers used by LVGL
    ESP_UTILS_LOGI("LVGL display init: reading panel size");
    auto lcd_width = lcd->getFrameWidth();
    auto lcd_height = lcd->getFrameHeight();
    int buffer_size = 0;
    ESP_UTILS_LOGI("LVGL display init: panel size %dx%d", lcd_width, lcd_height);
    ...
    for (int i = 0; (i < LVGL_PORT_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX); i++) {
        ESP_UTILS_LOGI("LVGL display init: allocating buffer[%d], pixels=%d", i, buffer_size);
        lvgl_buf[i] = heap_caps_malloc(buffer_size * sizeof(lv_color_t), LVGL_PORT_BUFFER_MALLOC_CAPS);
        ESP_UTILS_CHECK_NULL_RETURN(lvgl_buf[i], nullptr, "LVGL buffer allocation failed");
        ESP_UTILS_LOGD("Buffer[%d] address: %p, size: %d", i, lvgl_buf[i], buffer_size * sizeof(lv_color_t));
        ESP_UTILS_LOGI("LVGL display init: buffer[%d]=%p", i, lvgl_buf[i]);
    }
#else
    ...
    ESP_UTILS_LOGI("LVGL display init: draw buffer init");
    lv_disp_draw_buf_init(&disp_buf, lvgl_buf[0], lvgl_buf[1], buffer_size);

    ESP_UTILS_LOGI("LVGL display init: driver init");
    ...
    ESP_UTILS_LOGI("LVGL display init: registering driver");
    lv_disp_t *registered = lv_disp_drv_register(&disp_drv);
    ESP_UTILS_LOGI("LVGL display init: registered driver=%p", registered);
    return registered;
}
...
bool lvgl_port_init(LCD *lcd, Touch *tp)
{
    ...
    ESP_UTILS_LOGI("LVGL display driver initialized");
    // Default rotation is already none. Avoid calling lv_disp_set_rotation()
    // during first bench bring-up because it can trigger an early refresh path.
    ESP_UTILS_LOGI("LVGL bus type=%d", bus_type);
    ...
    if (bus_type != ESP_PANEL_BUS_TYPE_RGB) {
        ESP_UTILS_LOGI("LVGL attaching non-RGB draw finish callback");
        ...
        ESP_UTILS_LOGI("LVGL non-RGB draw finish callback attached");
    }

    if (tp != nullptr) {
        ESP_UTILS_LOGI("LVGL touch input init start");
        ESP_UTILS_LOGD("Initialize LVGL input driver");
        indev = indev_init(tp);
        ESP_UTILS_CHECK_NULL_RETURN(indev, false, "Initialize LVGL input driver failed");
        ESP_UTILS_LOGI("LVGL touch input init done");
    ...
    ESP_UTILS_LOGI("LVGL creating mutex");
    ESP_UTILS_LOGD("Create mutex for LVGL");
    lvgl_mux = xSemaphoreCreateRecursiveMutex();
    ESP_UTILS_CHECK_NULL_RETURN(lvgl_mux, false, "Create LVGL mutex failed");
    ESP_UTILS_LOGI("LVGL mutex created");

    ESP_UTILS_LOGI("LVGL task disabled for first bench demo; main loop drives lv_timer_handler()");

#if LVGL_PORT_AVOID_TEAR
    lcd->attachRefreshFinishCallback(onLcdVsyncCallback, (void *)lvgl_task_handle);
#endif

    ESP_UTILS_LOGI("LVGL port init complete");
    return true;
}
