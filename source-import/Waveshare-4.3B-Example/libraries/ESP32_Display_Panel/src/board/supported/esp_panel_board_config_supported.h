...
/*
 * Project lock: this firmware targets Waveshare ESP32-S3-Touch-LCD-4.3B.
 * PlatformIO's library include order can miss the Arduino-style
 * esp_panel_board_supported_conf.h file, so keep the default board selection
 * inside the internal config path that every board source includes.
 */
#undef ESP_PANEL_BOARD_DEFAULT_USE_SUPPORTED
